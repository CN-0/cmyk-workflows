...
const { requireRole } = require('../../../shared/src/middleware/auth');
...
// Update workflow
router.put('/:id', validateParams(idSchema), validateBody(updateWorkflowSchema), async (req, res) => {
  try {
    const { id } = req.params;
    const userId = req.user.userId;
    ...
    const currentVersion = existingResult[0].version;
...
// Duplicate workflow
router.post('/:id/duplicate', validateParams(idSchema), async (req, res) => {
  try {
    const { id } = req.params;
    const userId = req.user.userId;
    const db = req.app.locals.db;
