...
class NodeExecutor {
  ...
  executeWebhookTrigger(node, context) {
    // Webhook trigger - return the trigger data
    return context.triggerData;
  }

  executeScheduleTrigger(node, context) {
    // Schedule trigger - return current timestamp
    return { timestamp: new Date().toISOString() };
  }

  executeEmailTrigger(node, context) {
  ...
  async executeDatabaseInsert(node, context) {
  ...
  executeTransform(node, context) {
    const { input } = this.resolveInputs(node, context);
    const config = node.config || {};

    const code = config.code;

    // Simple code execution (in production, use a proper sandbox)
    try {
      const func = new Function('input', 'context', code);
      const result = func(input, context.nodeOutputs);
