...
const NodeComponent: React.FC<NodeComponentProps> = ({ node, onUpdate, onDelete }) => {
...
const WorkflowCanvas: React.FC<WorkflowCanvasProps> = ({ nodes, onNodesChange }) => {
  ...
  const handleDragEnd = useCallback((event: DragEndEvent) => {
    ...
    if (over?.id === 'workflow-canvas' && active.data.current) {
      ...
      if (typeof active.data.current === 'string') {
        try {
          const template: NodeTemplate = JSON.parse(active.data.current);
          const newNode: WorkflowNode = {
            id: `node-${Date.now()}`,
            type: template.type,
            label: template.name,
            position: { 
              x: Math.max(0, event.activatorEvent ? (event.activatorEvent as MouseEvent).clientX - 400 : 100), 
              y: Math.max(0, event.activatorEvent ? (event.activatorEvent as MouseEvent).clientY - 100 : 100) 
            },
            data: {},
          };
